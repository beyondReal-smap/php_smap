...
if ($_POST['act'] == "recom_list") {
    ...
    // 로그 파일의 경로를 정의합니다
    $log_file = '/data/wwwroot/app2.smap.site/error_logs/logfile.log';
    ...
    if ($cached_data === null) {
        ...
        log_to_file("sch point array: " . json_encode($arr_data, JSON_PRETTY_PRINT), $log_file);
        ...
        log_to_file("loc point array: " . json_encode($arr_data, JSON_PRETTY_PRINT), $log_file);
        ...
        if (!empty($loc_new)) {
            ...
            foreach ($mevents as $index => &$event) {
                ...
                if ($event['logStatus'] == 'stay') {
                    ...
                    $content = '<div class="point_wrap point2" data-rangeindex="' . $event['totalLogCount'] . '">
                                ...
                                // log_to_file("contentsss: " . $content, $log_file);
                } else {
            ...
            // log_to_file("Final events array: " . json_encode($mevents, JSON_PRETTY_PRINT), $log_file);

            // JSON으로 변환한 후 다시 배열로 디코딩
            $decodedEvents = json_decode(json_encode($mevents), true);

            // 배열의 인덱스를 재설정
            $tempEvents = array_values($decodedEvents);

            // $arr_data 배열에 각 이벤트의 정보를 담습니다.
            // $arr_data = [];
            foreach ($tempEvents as $index => $event) {
                ...
                // log_to_file("contentsss_" . ($index + 1) . ": " . $event['content'], $log_file);
            }
            ...
            foreach ($arr_data as $key => $value) {
                if (strpos($key, 'logmarkerLat_') === 0) {
                    $index = substr($key, strlen('logmarkerLat_'));
                    $lat = $value;
                    $long = $arr_data["logmarkerLong_$index"];
                    $content = $arr_data["logmarkerContent_$index"];

                    // 좌표와 내용을 기준으로 고유 키 생성
                    $uniqueKey = "$lat|$long|$content";

                    // 중복되지 않은 경우에만 추가
                    if (!isset($uniqueMarkers[$uniqueKey])) {
                        $uniqueMarkers[$uniqueKey] = [
                            "lat" => $lat,
                            "long" => $long,
                            "content" => $content
                        ];
            ...
            // 중복이 제거된 데이터로 $arr_data 재구성
            // $arr_data = [];
            $index = 1;
            foreach ($uniqueMarkers as $marker) {
                $arr_data["logmarkerLat_$index"] = $marker['lat'];
                $arr_data["logmarkerLong_$index"] = $marker['long'];
                $arr_data["logmarkerContent_$index"] = $marker['content'];
                $index++;
            }

            $arr_data['log_count'] = count($uniqueMarkers);
            $arr_data['log_chk'] = 'Y';
        } else {
    ...
    log_to_file("loc_new array: " . json_encode($loc_new, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Initial events array: " . json_encode($events, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Filtered events: " . json_encode($filteredEvents, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Events after removing moves within stay periods: " . json_encode($filteredEventsFinal, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Events after merging stays: " . json_encode($mergedStayEvents, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Final events array: " . json_encode($mevents, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Initial arr_data array: " . json_encode($arr_data, JSON_PRETTY_PRINT), $log_file);
    log_to_file("Final arr_data array after removing duplicates: " . json_encode($arr_data, JSON_PRETTY_PRINT), $log_file);
