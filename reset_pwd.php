...
if (isset($token)) {
    // 데이터베이스에서 토큰 조회
    $DB->where('mt_reset_token', $token);
    $tokenData = $DB->getOne('member_t');

    if ($tokenData) {
        // 토큰 만료 시간 확인
        $expirationTime = strtotime($tokenData['mt_token_edate']);
        if (time() > $expirationTime) {
            echo "<script>jalert_url('" . $translations['txt_coupon_applied'] . "', './login');</script>"; // "토큰이 만료되었습니다. 다시 비밀번호 재설정을 요청해주세요."
            exit;
        }
    } else {
        echo "<script>jalert_url('" . $translations['txt_invalid_token'] . "', './login');</script>"; // "유효하지 않은 토큰입니다."
        exit;
    }
} else {
    echo "<script>jalert_url('" . $translations['txt_invalid_access'] . "', './login');</script>"; // "잘못된 접근입니다."
    exit;
}
?>
<style>
            ...
            $.ajax({
                ...
                success: function(d, s) {
                    ...
                    if (d.result == "_ok") {
                        ...
                        location.replace('./login?email=' + mt_email);
                    } else {
                        console.log("error:" + d);
