    ...
    <div class="floating_wrap on">
        <div class="flt_inner">
            ...
            <div class="flt_body pb-5 pt-3">
                ...
                <p class="text_dynamic line_h1_3 text_gray fs_14 mt-2 fw_500">그룹원의 이동경로를 로그 메뉴에서 확인할 수 있습니다.
    ...
    <div class="modal-dialog modal-default modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center pb-4">
                ...
                <p class="fs_16 text_dynamic fw_700 line_h1_3 mt-4">로그를 확인할 때 가끔씩 광고가 뜰 거예요.
                    잠깐 시간 내주시면 큰 도움 됩니다.
                    감사해요!
                </p>
            </div>
            <div class="modal-footer w-100 px-0 py-0 mt-0 border-0">
                <div class="d-flex align-items-center w-100 mx-0 my-0">
                    ...
                    <button type="button" class="btn btn-primary btn-md w-50 rounded_t_left_0 rounded_t_right_0 rounded_b_left_0" onclick="show_ad_btn('<?= $_SESSION['_mt_idx'] ?>','3')">광고보기</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function show_ad_btn(mt_idx, log_count) {
        $('#showAd_modal').modal('hide');
        var form_data = new FormData();
        form_data.append("act", "show_ad_log");
        form_data.append("log_count", log_count);
    ...
    function f_member_location_info(status) {
        ...
        form_data.append("event_start_date", $('#event_start_date').val());
        form_data.append("ad_status", status);
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data) {
                    if (data.ad_alert == 'Y' && data.ad_show == 'Y') { // 알럿띄우고 광고봐야할때
                        $('#showAd_modal').modal('show');
                    } else {
                        if (data.ad_show == 'Y') { // 광고봐야할때 카운트 추가하고 광고추가하기
                            show_ad_btn(<?= $_SESSION['_mt_idx'] ?>, data.ad_count);
                        } else {
