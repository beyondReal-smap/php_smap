    ...
    if ($_POST['sst_idx']) {
        ...
        if ($_SESSION['_mt_idx'] == $sgdt_row['mt_idx']) { //해당 일정이 본인 일정일 경우
            if ($sgdt_row['sgdt_owner_chk'] == 'Y' || $sgdt_row['sgdt_leader_chk'] == 'Y') { // 본인이 오너 or 리더일 경우
                ...
                $DB->where('(sgdt_owner_chk ="Y" or sgdt_leader_chk="Y")');
    ...
    if ($cnt < 1) {
        ...
        if ($list_sst) {
            ...
            foreach ($list_sst as $row_sst_a) {
                ...
                // if (get_date_ttime($current_date) >= get_date_ttime($row_sst_a['sst_edate'])) {
                if ($row_sst_a['sst_all_day'] == 'Y') {
                    $content = '<div class="point_wrap point2"><button type="button" class="btn point point_ing"><span class="point_inner"><span class="point_txt">' . $count . '</span></span></button><div class="infobox rounded-sm bg-white px_08 py_08 on"><p class="fs_12 fw_500 text_dynamic line_h1_2">' . $row_sst_a['sst_title'] . '</p><p class="fs_8 text_dynamic mt-2">' . $sst_all_day_t . '</p></div></div>';
                    $status = "point_ing";
                } else if ($current_date >= $row_sst_a['sst_edate']) {
                    $content = '<div class="point_wrap point1"><button type="button" class="btn point point_done"><span class="point_inner"><span class="point_txt">' . $count . '</span></span></button><div class="infobox rounded-sm bg-white px_08 py_08 on"><p class="fs_12 fw_500 text_dynamic line_h1_2">' . $row_sst_a['sst_title'] . '</p><p class="fs_8 text_dynamic mt-2">' . $sst_all_day_t . '</p></div></div>';
                    $status = "point_done";
                } else if ($current_date >= $row_sst_a['sst_sdate'] && $current_date <= $row_sst_a['sst_edate']) {
                    $content = '<div class="point_wrap point2"><button type="button" class="btn point point_ing"><span class="point_inner"><span class="point_txt">' . $count . '</span></span></button><div class="infobox rounded-sm bg-white px_08 py_08 on"><p class="fs_12 fw_500 text_dynamic line_h1_2">' . $row_sst_a['sst_title'] . '</p><p class="fs_8 text_dynamic mt-2">' . $sst_all_day_t . '</p></div></div>';
                    $complete_chk = true;
                    ...
                    $content = '<div class="point_wrap point3"><button type="button" class="btn point point_gonna"><span class="point_inner"><span class="point_txt">' . $count . '</span></span></button><div class="infobox rounded-sm bg-white px_08 py_08 on"><p class="fs_12 fw_500 text_dynamic line_h1_2">' . $row_sst_a['sst_title'] . '</p><p class="fs_8 text_dynamic mt-2">' . $sst_all_day_t . '</p></div></div>';
                    $status = "point_gonna";
                }
