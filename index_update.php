...
if ($_POST['act'] == "weather_get") {
    ...
    if ($_SESSION['_mt_idx']) {
        ...
        $_SESSION['mt_sido'] = $region['area1'];
        $_SESSION['mt_gu'] = $region['area2'];
        $_SESSION['mt_dong'] = $region['area3'];
    }
                    ...
                    if ($list_sst_a) {
                        foreach ($list_sst_a as $row_sst_a) {
                            ...
                            if ($gpd_d < 0.1) { //100m이내면 체류중
                                ...
                                $arr_rtn['speed_km'] = $translations['txt_staying'];
                            }

                            if ($qw == 1) {
                                $arr_rtn['my_add_next'] = $row_sst_a['sst_location_add'];
                                $arr_rtn['my_arrive_next'] = $translations['txt_expected'];
                                $arr_rtn['my_time_next'] = $translations['txt_time_example_pm'];
                            } else {
                                $arr_rtn['my_add_next'] = '';
                            ...
                            <a href="./location?sgdt_mt_idx=<?= $row_slmt['sgdt_mt_idx'] ?>" class="d-flex align-items-center">
                                <div class="prd_img flex-shrink-0 mr_12">
                                    <div class="rect_square rounded_14">
                                        <img src="<?= $mt_file1_url ?>" onerror="this.src='<?= $ct_no_profile_img_url ?>'" alt="<?= $translations['txt_image'] ?>" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="d-flex align-items-center flex-wrap">
                                        <?php
                                        if ($arr_rtn['speed_info'] == $translations['txt_moving']) {
                                        ?>
                    ...
                    <div class="mt-3">
                        <div class="bg-white d-flex align-items-start justify-content-between border rounded-lg py_16 px-2">
                            <div class="text-center w_50 pr-2 border-right">
                                <p class="fs_14 fw_500"><?= $translations['txt_current_location'] ?></p>
                                ...
                                <div class="fc_primary rounded-pill bg_secondary mx-auto text-center px_08 py_03 text_dynamic w_fit h_fit_im d-flex flex-wrap align-items-center justify-content-center mt_08 re_time_txt">
                                    <p class="fs_12 pr_03 d-none"><?= $translations['txt_arrived'] ?> ·</p>
                                    <p class="fs_12 d-none"><?= $translations['txt_time_example_pm_2'] ?></p>
                                </div>
                            </div>
                            <div class="text-center w_50 pl-2">
                                <p class="fs_14 fw_500 text_gray "><?= $translations['txt_next_location'] ?></p>
        ...
        <div class="mt-4 pb-4">
            <div class="border rounded-lg px_16 py_16 none_box mb_25">
                <div class="text-center">
                    <p class="fs_14 text_gray text_dynamic"><?= $translations['txt_create_group_exclamation'] ?></p>
                    <button type="button" class="btn btn-secondary btn-sm fc_primary pl_14 pr_11 mt_12 mx-auto" onclick="location.href='./group_create'"><?= $translations['txt_create_group_button'] ?><i class="xi-angle-right-min ml_19"></i></button>
                </div>
            </div>
        </div>
<?php
