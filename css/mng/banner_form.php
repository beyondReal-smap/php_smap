...
if ($_GET['act'] == "update") {
    ...
    if($row['bt_file']) {
        ...
        $arr_images[1][]  = array(
            'name' => $row['bt_file_ori'],
            'url' => $ct_img_url.'/'.$row['bt_file'],
            'size' => $row['bt_file_size'],
            'bt_idx' => $row['bt_idx'],
        );
...
<div class="content-wrapper">
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">배너관리<?=$_act_txt?></h4>
                    ...
                    <form method="post" name="frm_form" id="frm_form" action="./banner_update" target="hidden_ifrm" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            <label for="bt_type" class="col-sm-2 col-form-label">구분 <b class="text-danger">*</b></label>
                            <div class="col-sm-2">
                                <select name="bt_type" id="bt_type" class="form-control form-control-sm">
                        ...
                        <div class="form-group row">
                            <label for="bt_title" class="col-sm-2 col-form-label">내용 <b class="text-danger">*</b></label>
                            <div class="col-sm-10">
                                <input type="text" name="bt_title" id="bt_title" value="<?=$row['bt_title']?>" class="form-control form-control-sm" maxlength="100" />
                            </div>
                        </div>
...
?>
                        ...
                        <div class="form-group row">
                            <label for="bt_target" class="col-sm-2 col-form-label">URL <b class="text-danger">*</b></label>
                            <div class="col-sm-10 form-inline">
                                <select name="bt_target" id="bt_target" class="form-control form-control-sm mr-3">
                                    <option value="1">새창</option>
                                    <option value="2">현재창</option>
                                </select>
                                <input type="text" name="bt_url" id="bt_url" value="<?=$row['bt_url']?>" class="form-control form-control-sm" maxlength="200" style="width:80%;" placeholder="http://, https:// 를 포함하여 입력바랍니다." />
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-2">
                                <input type="text" name="bt_rank" id="bt_rank" value="<?=$row['bt_rank']?>" class="form-control form-control-sm" numberOnly maxlength="5" />
                                <small id="bt_rank_help" class="form-text text-muted">* 낮을수록 상위 노출됩니다.</small>
                            </div>
                        </div>
                        <div class="form-group row">
                    ...
                    <?php if ($row['bt_type']) { ?>
                    $('#bt_type').val('<?=$row['bt_type']?>');
                    <?php } ?>
                    <?php if ($row['bt_show']) { ?>
                    $('#bt_show').val('<?=$row['bt_show']?>');
                    <?php } ?>
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    if (data == 'Y') {
                                        ...
                                        <?php } else { ?>
                                        jalert_url('등록되었습니다.', './banner_list');
                                        <?php } ?>
                                    }
                                },
                                error: function(err) {
