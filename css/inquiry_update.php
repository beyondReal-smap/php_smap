...
<div id="accordion" class="accordion_1">
    ...
    if ($list) {
        foreach ($list as $row) {
            if($row['qt_status']=='1') {
                $qt_status_cls = 'text_gray';
            } else {
                $qt_status_cls = 'text-primary';
            }
            ?>
...
<?php
    ...
    if($_POST['qt_qtitle'] == "") {
        p_alert("잘못된 접근입니다. qt_qtitle", 'back');
    }
    if($_POST['qt_qcontent'] == "") {
        p_alert("잘못된 접근입니다. qt_qcontent", 'back');
    }
