...
<div class="container sub_pg">
    <div class="mt-4">
        ...
        <form method="post" name="frm_form" id="frm_form" action="./setting_update" target="hidden_ifrm" enctype="multipart/form-data">
            ...
            <div class="b_botton">
                <button type="submit" class="btn w-100 rounded btn-primary btn-lg btn-block "><?= $translations['txt_check_email_link'] ?></button>
            </div>
        ...
        <script>
            ...
            function f_withdraw() {
                ...
                if (c < 1) {
                    jalert('<?= $translations['txt_select_withdraw_reason'] ?>');
                    return false;
                }
                if ($("input:radio[name=mt_retire_chk]:checked").val() == '4' && $('#mt_retire_etc').val() == '') {
                    jalert('<?= $translations['txt_enter_other_reason'] ?>');
                    return false;
                }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 'Y') {
                            ...
                            jalert_url('<?= $translations['txt_withdraw'] ?>', './join_entry');
                        } else {
...
<div class="modal fade" id="withdraw_modal" tabindex="-1">
    <div class="modal-dialog modal-default modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="bg_gray px_16 pt_16 pb-2">
